...
class AWACTrainer(TorchTrainer):
    def __init__(
        ...
        self.eval_policy = MakeDeterministic(self.policy) # this class is only contains get_action()
    ...
    def train_from_torch(self, batch, train=True, pretrain=False,):
        ...
        if self.brac:
            ...
            buf_log_pi = buf_dist.log_prob(actions)  # I need to implement log_prob in GaussianPolicy
        ...
        if self.awr_sample_actions:
            ...
            buf_dist = self.buffer_policy(obs)
            u, _ = buf_dist.rsample_and_logprob() # I need to implement rsample_and_logprob() in GaussianPolicy
        ...
        if self.normalize_over_state == "advantage":
            ...
            for i in range(K):
                ...
                log_b = buffer_dist.log_prob(u) # I need to implement log_prob() in GaussianPolicy
